...
const FormTable = () => {
  ...
  return (
    <React.Fragment>
      ...
      <Link to='terminationlistpage'><Button className='ui violet inverted' >Go To Termination Report</Button></Link>
      {openStatus.map(item => (

        <div key={item._id} id='formbtn'>
          <Link to='employeechangeForm'><Button size='large' color='violet' onClick={(e) => storeDbId(e)} value={item._id}><Icon name='file' /> Continue with Change Form In Progress</Button></Link>
        </div>
        ...
        <Table.Body>
          ...
          {closedStatus.map(history => (
            <Table.Row key={history._id}>
              ...
              <Table.Cell>Employee Change Form</Table.Cell>
